...
import { Archive, BadgeCheck, Building2, Copy, FileSearch, ShieldCheck } from "lucide-react";
...
export const metadata = pageMetadata({
  title: "Giải pháp máy photocopy theo ngành, nhu cầu & thương hiệu",
  description:
    "Tổng hợp máy photocopy HPT Tech theo ngành, theo nhu cầu như photocopy màu, tốc độ cao, khổ A3 và theo thương hiệu Fujifilm, Canon, Sharp, Ricoh, Konica Minolta. Tư vấn & báo giá.",
  path: "/giai-phap/may-photocopy",
});
  ...
  return (
    <div className="mx-auto max-w-[1200px] px-5">
      <nav className="py-4 text-sm text-slate-600" aria-label="Breadcrumb">
        ...
        <span className="px-2">/</span>
        <span className="font-semibold text-primary-700">Máy photocopy</span>
      </nav>
    </div>
  );
}
...
export default async function PhotocopierSolutionsPage() {
  const { photocopy } = await getHubData();
  const total = photocopy.industry.length + photocopy.need.length + photocopy.brand.length;
      ...
      <section className="border-y border-border bg-gradient-to-br from-primary-50 via-white to-surface">
        <div className="mx-auto max-w-[1200px] px-5 py-12">
          ...
          <h1 className="mt-4 max-w-4xl text-4xl font-extrabold leading-tight tracking-normal text-ink md:text-5xl">
            Giải pháp máy photocopy theo ngành, nhu cầu & thương hiệu
          </h1>
          ...
          <div className="mt-8 inline-flex items-center gap-2 rounded-md border border-primary-100 bg-white px-4 py-3 text-sm font-semibold text-slate-700">
            ...
            {total} landing đang publish trong hub máy photocopy
          </div>
        </div>
      </section>
      ...
      <div className="mx-auto max-w-[1200px] px-5 py-12">
        {total === 0 ? (
          <div className="mx-auto max-w-2xl rounded-2xl border border-border bg-surface p-10 text-center">
            <p className="text-lg font-semibold text-ink">Các trang giải pháp máy photocopy đang được biên soạn.</p>
            <p className="mt-2 leading-7 text-slate-600">
              Trong lúc chờ, bạn có thể xem toàn bộ danh mục máy photocopy chính hãng hoặc gọi trực tiếp để được tư vấn theo nhu cầu.
            </p>
            <Link
              href="/may-photocopy"
              className="mt-5 inline-flex h-12 items-center rounded-md bg-primary-700 px-6 text-sm font-bold text-white transition hover:bg-primary-800"
            >
              Xem danh mục máy photocopy
            </Link>
          </div>
        ...
        <Section title="Theo ngành" count={photocopy.industry.length}>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-5">
            {photocopy.industry.map((item) => <IndustryCard key={item.pathname} item={item} />)}
          </div>
        </Section>

        <Section title="Theo nhu cầu" count={photocopy.need.length}>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-5">
            {photocopy.need.map((item) => <CompactCard key={item.pathname} item={item} />)}
          </div>
        </Section>

        <Section title="Theo thương hiệu" count={photocopy.brand.length}>
          <div className="flex flex-wrap gap-2">
            {photocopy.brand.map((item) => (
              <Link
                key={item.pathname}
                href={item.pathname}
              ...
              >
                {item.title.replace(/^Máy photocopy\s+/i, "")}
              </Link>
            ))}
          </div>
        </Section>
