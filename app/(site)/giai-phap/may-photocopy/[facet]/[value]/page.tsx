...
export async function generateMetadata({ params }: RouteContext): Promise<Metadata> {
  ...
  const page = await getLandingPageByPath(`/giai-phap/may-photocopy/${facet}/${value}`);
  if (!page) return {};
...
export default async function PhotocopierLandingPage({ params }: RouteContext) {
  const { facet, value } = await params;
  if (!isFacetSegment(facet)) notFound();

  const page = await getLandingPageByPath(`/giai-phap/may-photocopy/${facet}/${value}`);
  if (!page || page.productGroup !== "may-photocopy") notFound();
