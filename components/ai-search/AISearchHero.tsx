...
export default function AISearchHero({
  ...
  return (
    <section className="overflow-hidden rounded-lg bg-[#061b63] px-4 py-8 text-white shadow-[0_24px_70px_-45px_rgba(15,23,42,0.8)] md:px-8 md:py-10">
      <div className="mx-auto max-w-6xl">
        <div className="flex flex-col items-center text-center">
          <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.14em] text-blue-100">
            <Bot size={16} />
            ...
            return (
              <div key={item.label} className="rounded-lg border border-white/10 bg-white/10 px-4 py-3">
                <div className="flex items-start gap-3">
                  <Icon className="mt-0.5 text-blue-200" size={18} />
                  <div>
                    <p className="text-sm font-bold text-white">{item.label}</p>
                    <p className="mt-1 text-xs text-blue-100">{item.value}</p>
                  </div>
                </div>
              </div>
            );
