...
export default function CompareDock({
  ...
  const compareHref = `/compare?products=${encodeURIComponent(items.map((p) => p.id).join(","))}`;
        ...
        <div className="compare-drawer-body">
          <div className="compare-slot-grid">
            {slots.map((item, index) =>
              item ? (
                <article className="compare-slot filled" key={item.id}>
              ...
              {pickerQuery.trim().length < 3 ? (
                ...
                <div className="compare-picker-results">
                  {pickerResults.map((product) => (
                    <button className="compare-picker-item" type="button" key={product.id} onClick={() => {
                      onAdd(product);
                      setPickerOpen(false);
                    }}>
