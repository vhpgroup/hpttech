...
| Hình ảnh | Không crawl, không download, không upload; nhân viên tự thêm trong CMS |
| Trạng thái | Mọi sản phẩm do batch tạo hoặc cập nhật đều là `draft` |
| Publish | Máy scan cần đạt ngưỡng `scannerSpecs` tối thiểu 21/24 cột; ảnh vẫn do nhân viên bổ sung và review |
...
| Ảnh | Không nhập; `images` giữ rỗng |
