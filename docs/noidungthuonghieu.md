# Nội dung bài viết chi tiết — 4 chứng nhận (dán vào field content) · HPT Tech

## Cách dán vào Payload

Mỗi hãng dưới đây gồm 2 phần:
- **Tóm tắt** → dán vào field `summary` (dùng cho card + meta description, nên giữ ngắn).
- **Bài viết** (markdown) → dán vào field `content` (rich text).

**Mẹo dán rich text (Lexical của Payload):**
- Editor Lexical thường tự nhận markdown: gõ/dán `## ` → Heading 2, `### ` → Heading 3, `**đậm**` → in đậm, `- ` → bullet, `> ` → trích dẫn.
...
```md
## HPT Tech — Nhà phân phối độc quyền máy scan Microtek tại Việt Nam
...
```md
## HPT Tech — Đối tác chính thức Omada & VIGI của TP-Link Việt Nam
...
```md
## HPT Tech — Nhà phân phối ủy quyền Joyusing tại Việt Nam
...
```md
## HPT Tech — Nhà phân phối ủy quyền TTR Entertainment tại Việt Nam
